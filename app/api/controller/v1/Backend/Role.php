...
class Role extends Base
{
    ...
    public function create(){
        ...
        $result = $this->validate($this->data, 'AddRole');
        if($result !== true){
    ...
    public function remove(){
        ...
        if($data['a_deleteTime'] == 0){
            $type  = '禁用';
            $state = -1;
            $value = ['a_state' => $state, 'a_deleteTime' => time()];
        }else{
            $type = '启用';
            $state = 0;
    ...
    public function publish(){
        ...
        $result = Db::name('article_post')->where(['a_id' => $id])->update(['a_publishedTime' => time(), 'a_state' => 1]);
